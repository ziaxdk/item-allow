...
const File = require('./data/countries/c50.json');
...
var bulkAll = function(cb) {
  console.log(`Countries to index: ${body.length/2}`);
  client.bulk({
    body: body
  }, function(err, res) {
    if (err) return err;
    if (res.errors) {
      let errors = res.items.filter(doc => doc.index.status != 200).map(doc => { return { id: doc.index._id, error: doc.index.error }});
      console.log(inspect(errors, { depth:null }));
    }
    cb();
  });
}
